...
function ModulePanel({ title, description, productName, onOpen, disabled, icon }) {
  return (
    <div style={{
      ...
      <div>
        <div style={{ fontWeight: 600, fontSize: 15, color: "var(--color-text)", marginBottom: 6 }}>
          {title}
        </div>
      ...
      <button
        ...
        style={{
          ...
          background: disabled ? "#e2e8f0" : "var(--color-brand)",
...
export default function SparePartsTab({ data, projectId }) {
  ...
  const openModule = () => {
    history.push({
      ...
      state: { projectId, productId, productName: data?.productName },
    });
  };

  return (
    <div className="form-section">
      <div className="form-section__header">Spare Parts</div>
      <div className="form-section__body">
        <ModulePanel
          title="Spare Parts Analysis"
          description="Spare parts analysis and provisioning for this component."
