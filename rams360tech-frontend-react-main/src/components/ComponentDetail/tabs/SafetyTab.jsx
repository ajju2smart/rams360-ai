...
export default function SafetyTab({ data, projectId }) {
  const history = useHistory();
  const productId = data?.id || data?.productId;
  const openModule = () => {
    history.push({ pathname: "/safety/" + projectId, state: { projectId, productId, productName: data?.productName } });
  };
  return (
    <div className="form-section">
      <div className="form-section__header">Safety</div>
      <div className="form-section__body">
        <ModulePanel title="Safety Analysis" description="Hazard identification and safety analysis for this component." productName={data?.productName} onOpen={openModule} disabled={!productId} icon="🛡️" />
      </div>
    </div>
  );
}
