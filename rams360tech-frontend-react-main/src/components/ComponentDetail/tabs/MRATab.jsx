...
export default function MRATab({ data, projectId }) {
  const history = useHistory();
  const productId = data?.id || data?.productId;
  const openModule = () => {
    history.push({ pathname: "/pmmra/" + projectId, state: { projectId, productId, productName: data?.productName } });
  };
  return (
    <div className="form-section">
      <div className="form-section__header">MRA / RCM</div>
      <div className="form-section__body">
        <ModulePanel title="MRA / RCM" description="Maintenance Requirements Analysis and Reliability Centred Maintenance." productName={data?.productName} onOpen={openModule} disabled={!productId} icon="🔧" />
      </div>
    </div>
  );
}
