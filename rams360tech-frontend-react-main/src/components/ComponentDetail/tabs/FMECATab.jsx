...
export default function FMECATab({ data, projectId }) {
  const history = useHistory();
  const productId = data?.id || data?.productId;
  const openModule = () => {
    history.push({ pathname: "/fmeca/" + projectId, state: { projectId, productId, productName: data?.productName } });
  };
  return (
    <div className="form-section">
      <div className="form-section__header">FMECA</div>
      <div className="form-section__body">
        <ModulePanel title="FMECA" description="Failure Mode, Effects and Criticality Analysis for this component." productName={data?.productName} onOpen={openModule} disabled={!productId} icon="⚠️" />
      </div>
    </div>
  );
}
